...
export default function EventsFeedItem({ post }) {
  ...
  switch (post.code) {
    case 'joined-event':
      summary = (
        <>
          <Link to={ `/profile/${ post.userUid }` }>{ post.displayName }</Link> has signed up to <Link
    ...
    case 'left-event':
      summary = (
        <>
          <Link to={ `/profile/${ post.userUid }` }>{ post.displayName }</Link> has cancelled place on <Link
          to={ `/events/${ post.eventId }` }>{ post.title }</Link>
        </>
      );
      break;
